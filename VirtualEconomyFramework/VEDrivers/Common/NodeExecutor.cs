...
{
    public class NodeExcecutorConfig
    {
    ...
    public class NodeExecutor : IHostedService
    {
        ...
        public NodeExecutor(IConfiguration settings, IHostApplicationLifetime lifetime)
        {
            var config = settings.GetSection("NodeExecutor").Get<NodeExcecutorConfig>();
            if (config == null)
            {
                log.Fatal("NodeExecutor section not found in config");
                throw new Exception();
            }
            fullPath = config.Exe;
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            process.Exited += (sender, e) =>
            {
                ...
                if (!Volatile.Read(ref monitoring))
                {
                    ...
                    if (string.IsNullOrEmpty(errors)) errors = process.StandardOutput.ReadToEnd(); //node red hlasi obsazeny socket na stdout
